...
{
    /// <summary>
    /// This is where all the controls processing takes place
    /// can be merged in the same classes but better to keep them
    /// seperate.
    /// </summary>

    public class WebControlAdapterExtender
    {
        private readonly WebControl _adaptedControl;

        private bool _disableAutoAccessKey;
        // used when dealing with things like read-only textboxes that should not have access keys
        //this makes use of CSS Friendly which can be downloaded from MSFT codeplex.
        ...
        public WebControl AdaptedControl
        {
            get
            {
                Debug.Assert(_adaptedControl != null, "Adapters internal error",
        ...
        public bool AdapterEnabled
        {
            get
            {
                const bool bReturn = true; // normally the adapters are enabled
        ...
        public bool AutoAccessKey
        {
            get
            {
                ...
                const bool bReturn = true; // by default, the adapter will make access keys are available
        ...
        public static void RemoveProblemTypes(ControlCollection coll, List<ControlRestorationInfo> stashedControls)
        {
            foreach (Control ctrl in coll)
            {
                if (ctrl is RequiredFieldValidator ||
                    ctrl is CompareValidator ||
                    ctrl is RegularExpressionValidator ||
                    ctrl is ValidationSummary)
                {
        ...
        {
            for (var index = 0; index < stashedControls.Count; index++)
            {
                var cri = stashedControls[index];
                cri.Restore();
            }
        }
        ...
        public void WriteLabel(HtmlTextWriter writer, string className, string text, string forId)
        {
            if (!String.IsNullOrEmpty(text))
            {
                ...
                if (AutoAccessKey)
                {
                    ...
                    writer.Write(HtmlTextWriter.TagRightChar);
                    writer.Write(text[0].ToString(CultureInfo.InvariantCulture));
                                 ...
                                 string inputClassName, string id, string value)
        {
            ...
            {
                writer.WriteAttribute("accesskey", labelText[0].ToString(CultureInfo.InvariantCulture).ToLower());
            }
        ...
        {
            var oldDisableAutoAccessKey = _disableAutoAccessKey;
                                  ...
                                  string id, bool isChecked)
        {
            ...
            if (AutoAccessKey && (!String.IsNullOrEmpty(labelText)))
            {
                writer.WriteAttribute("accesskey", labelText[0].ToString(CultureInfo.InvariantCulture));
            }
            writer.Write(HtmlTextWriter.SelfClosingTagEnd);
    ...
    {
        private readonly ControlCollection _coll;
